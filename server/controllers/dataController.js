  ...
  getData(req, res) {
    ...
    CacheModel.findOne({ id: process.env.CACHE_ID}, function(err, cache) {
      if (cache) {
        ...
        // check to see if the cache is fresh, update if not
        if (!dateHelpers.lastNHours(cache.date, 6)){
        // if (1 !== 2){
          try {
            http.get({
        ...
        console.log('[Data Controller] Error getting data from the cache', err);
      }
    });
  }
